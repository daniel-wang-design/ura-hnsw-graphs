...
class StaticHNSWIndexSequentialCreation(BasicHNSWIndex):
    """
    Static HNSW index:
    ...
    def create_index(self, initial_vectors: np.ndarray, **options: Any) -> None:
        ...
        for i in range(n):
            # x[i] is shape (d,) and C-contiguous because x is C-ordered
            idx.add_items(x[i], int(i), num_threads=num_threads)        
        idx.set_ef(ef)
